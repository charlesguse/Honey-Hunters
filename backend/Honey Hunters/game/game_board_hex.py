...
class GameBoardHex(gbb.GameBoardBase):
    ...
    def DisplayVisibleSpots(self, playerId, updateArr, refArray, x, y):
        if 0 <= x and x < len(refArray) and 0 <= y and y < len(refArray[x]) and len(refArray) == len(updateArr) and len(refArray[x]) == len(updateArr[x]) and updateArr[x][y] == gbb.GameBoardBase.NOT_VISIBLE:
            if refArray[x][y] == gbb.GameBoardBase.HONEY_SPOT and playerId == self.playerOne:
                self.playerOneScore += 1
            if refArray[x][y] == gbb.GameBoardBase.HONEY_SPOT and playerId == self.playerTwo:
                self.playerTwoScore += 1
            updateArr[x][y] = refArray[x][y]
            if refArray[x][y] == gbb.GameBoardBase.EMPTY_SPOT:
    ...
    def AddHoneyToTile(self, arr, x, y):
        arr[x][y] = gbb.GameBoardBase.HONEY_SPOT
        # Top (odd x and even x)
